		...
		if(cod.length > 10){
			...
			getProdutoCodBarras(cod, (data) => {
				if(data){
					...
					// $.get(path+'produtos/getProdutoCodigBarra/'+id)
					// .done((res) => {
...
function verificaProdutoInclusoAtalho(produto, call){
	let cont = 0;
	ITENS.map((rs) => {
		if(produto.id == rs.id){
...
function incrementaItem(id){
	...
	ITENS.map((v) => {
		...
		if(v.cont != id){
			...
			verificaProdutoInclusoAtalho(prod, (call) => {
...
function somaMultiplo(){
	...
	let soma = v1 + v2 + v3;
	$('#vl_restante').html(formatReal((TOTAL+VALORACRESCIMO - DESCONTO) - soma))
	if(soma == (TOTAL+VALORACRESCIMO - DESCONTO)){
		$('#btn-ok-multi').removeAttr('disabled')
	}else if(soma > (TOTAL+VALORACRESCIMO - DESCONTO)){
